...
result_df <- readRDS(file = paste0(here(), '/result_df_scores2D.rds'))
...
for(i in 1:length(box_list)) {
  tikz(file = paste0(here(), "/score_box_mu_",
                     box_titles[[i]], ".tex"),
       width = 5,
       height = 5,
       standAlone = TRUE)
...
left_join(cov_tab, width_tab, by = c("n", "gamma", "b", "group")) |>
  mutate(n = as.integer(n)) |>
  (\(x) print(xtable(x, digits = 4), include.rownames=FALSE))()
